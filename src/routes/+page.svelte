<script>
  ...
  let chatting = false;
    ...
    // Push an empty assistant message (typing bubble)
    messages = [...messages, { role: 'assistant', text: '' }];
    const idx = messages.length - 1; // index of assistant message
    ...
    try {
      ...
      if (!res.ok) {
        ...
        while (true) {
          ...
          // On first token, ensure we scroll into view
          if (firstChunk) {
            firstChunk = false;
            await tick();
            const el = document.querySelector('.icl-chat__messages');
